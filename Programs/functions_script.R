...
# Function that creates a plot of the inertia corrected temperature and the measured temperature without inertia correction
thermal_inertia_plot = function(landcover_dataframe){
  png("/home/mivieijra/Documents/VLINDER/VLINDER/Produced_data/thermal_inertia.png")
  plot = ggplot() + geom_line(data = landcover_dataframe, aes(x = fulldate, y = measured_temp, color = "Measured temperature")) + geom_line(data = landcover_dataframe, aes(x = fulldate, y = thermal_inertia, color = "Inertia corrected temperature")) 
  plot2 = plot + scale_color_manual(values = c("Measured temperature" = "black", "Inertia corrected temperature" = "red")) 
  plot3 = plot2 + labs(x = 'Time (in UTC)', y = 'Temperature (°C)') + theme(plot.title = element_text(hjust = 0.5), legend.position="top", legend.title=element_blank(), legend.text=element_text(size=17), axis.title=element_text(size=17), axis.text=element_text(size=17)) + guides(color=guide_legend(ncol=2))
  print(plot3)
  dev.off()
}
...
# Function that creates a dataframe with the smoothed temperature residuals for a set of weather stations 
create_temp_residual_station_df = function(landcover_dataframe, station_directory_list, station_name_list){
  
  # Calculate the start and end time of the bicycle measurements
  starttime = min(landcover_dataframe$fulldate)
  endtime = max(landcover_dataframe$fulldate)
  
  # Create a 'time' column in the dataframe
  dataframe_residuals_stations = data.frame("date" = landcover_dataframe$fulldate)
  
  # Add the smoothed temperature residuals to the dataframe for each of the weather stations
  for (index in 1:length(station_name_list)){
    # Read the data from the datafile of the weather station
    stationdata = read.table(station_directory_list[index], header = FALSE, sep = ',', skip = 1)
    names(stationdata) = c("fulldate", "temperature")
    stationdata$fulldate = as.POSIXct(stationdata$fulldate, format="%Y-%m-%d %H:%M:%S", tz=Sys.timezone(), origin)
    stationdata$fulldate = as.POSIXct(stationdata$fulldate, format="%Y-%m-%d %H:%M:%S", tz="UTC")
    ...
    # Calculate the smoothed temperature residuals for the particular weather station
    temp_res = interpolated_temp[1]-interpolated_temp
    station_smoothed_residuals = smooth.spline(landcover_dataframe$fulldate, temp_res, df=5)
    smoothed_residuals = station_smoothed_residuals$y
    
    # Add the smoothed temperature residuals to the dataframe
    dataframe_residuals_stations = add_column(dataframe_residuals_stations, !!(paste0("residu_", station_name_list[index])) := smoothed_residuals)
  }
  
  return(dataframe_residuals_stations)
}

# Function that creates a plot of the temperatures corrected based on a land cover decline correction, a decline correction based on smoothed temperature residuals of one station and a decline correction based on the temperature difference between start and end time of the bicycle transect
# The temperature without decline correction is added as well
temp_decline_comparison = function(landcover_dataframe, station_directory_list, station_name_list, names, land_cover_decline_temp){
  
  # Create a dataframe with the smoothed temperature residuals of the set of weather stations
  station_residual_df = create_temp_residual_station_df(landcover_dataframe, station_directory_list, station_name_list)
  
  # Create a dataframe with the decline corrected temperatures based on the smoothed temperature residuals of one station for a particular set of weather stations
  final_data = data.frame("date" = landcover_dataframe$fulldate)
  for (index in 1:length(names)){
    
    temp_residuals = station_residual_df[,unlist(names[[index]])]
    
    corr_temp = landcover_dataframe$thermal_inertia + temp_residuals
    
    final_data = cbind(final_data, corr_temp)
  }
  
  # Add the decline corrected temperatures based on the temperature difference between start and end time of the bicycle transect
  temp_diff = landcover_dataframe$thermal_inertia[1] - landcover_dataframe$thermal_inertia[length(landcover_dataframe$thermal_inertia)]
  vector_residuals = seq(0, temp_diff, length.out = length(landcover_dataframe$thermal_inertia))
  
  corr_temp = landcover_dataframe$thermal_inertia + vector_residuals
  final_data = cbind(final_data, corr_temp)
  
  # Add the temperatures corrected based on a land cover decline correction
  final_data["land_cover_decline"] = land_cover_decline_temp
  # Add the temperatures not corrected for temperature decline
  final_data["no_decline"] = landcover_dataframe$thermal_inertia
  
  # For the Antwerp measurements
  names(final_data) = c("date", "VLINDER-Beveren", "AWV-Stabroek", "WOW-Borgerhout", "Infrabel-Berchem", "VLINDER-Kontich", "uniform_residual_decline", "land_cover_decline", "no_decline")
  # For the Ghent measurements
  #names(final_data) = c("date", "AWV-Evergem", "VLINDER-Observatory", "AWV-Oosterzele", "WOW-Destelbergen", "MOCCA-Wondelgem", "uniform_residual_decline", "land_cover_decline", "no_decline")
  
  melted_residual = melt(final_data, id.vars = "date")
  
  # For the Antwerp weather stations
  names_vector = c("VLINDER-Beveren", "AWV-Stabroek", "WOW-Borgerhout", "Infrabel-Berchem", "VLINDER-Kontich", "uniform mob. meas. residual decline", "land cover decline", "no decline")
  # For the Ghent weather stations
  #names_vector = c("AWV-Evergem", "VLINDER-Observatory", "AWV-Oosterzele", "WOW-Destelbergen", "MOCCA-Wondelgem", "uniform mob. meas. residual decline", "land cover decline", "no decline")
  
  color_vec = c("blue", "red", "cyan", "forestgreen", "green", "orange", "magenta", "black")
  
  # Plot the temperatures corrected based on the different temperature decline corrections
  png("/home/mivieijra/Documents/VLINDER/VLINDER/Produced_data/temp_decline_comparison.png", width=600, height=600)
  plot = ggplot() + geom_line(data = melted_residual, aes(x = date, y = value, color = variable))                                                                              
  plot2 = plot + scale_color_manual(labels = names_vector, values = color_vec) + labs(x = 'Time (in UTC)', y = 'Temperature (°C)') + guides(color = guide_legend(ncol = 2), linetype=FALSE) + theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank(), legend.position = "top", legend.text=element_text(size=17), axis.title=element_text(size=19), axis.text=element_text(size=19))
  print(plot2)
  dev.off()
  
}

# Function that corrects the temperatures for temperature decline based on land cover and a specific reference data point (reference time) and for a particular set of weather stations
...
# Function that calculates the temperatures corrected for temperature decline based on land cover at different times in the time range of the measurements
...
# The land cover is based on the BBK map of Flanders
temp_decline_land_cover_timeseries = function(landcover_dataframe, seconds_per_interval, landcover_weather_stations, station_directory_list, station_name_list, distance_vector){
  ...
  for (reference_number in vector_ref_numbers){
    print(paste0("Correcting for land cover temperature decline at reference time: ", landcover_dataframe$fulldate[reference_number], " based on the BBK land cover map of Flanders"))
...
# Function that calculates the temperatures corrected for temperature decline based on the smoothed temperature residuals of one station at different times in the time range of the measurements
# The begin time of the measurements is always included and the time interval is specified in the general settings in the main script by the 'seconds_per_interval' variable
# These will also be the times for which temperature charts will be made
temp_decline_traditional_timeseries = function(landcover_dataframe, station_residuals_df, name_station, seconds_per_interval){
  
  # Create a vector of the numbers of the reference data points that are used for different temperature decline corrections
  # The time interval between two consecutive reference data points is specified by the 'seconds_per_interval' variable
  vector_ref_numbers = seq(1, length(landcover_dataframe$fulldate), by = seconds_per_interval/10)
  
  # Extract the smoothed temperature residuals of the particular station
  smoothed_temp_residuals = station_residuals_df[,c(name_station)]
  
  # Perform the temperature decline for each reference time 
  for (reference_number in vector_ref_numbers){
    print(paste0("Correcting for traditional temperature decline at reference time: ", landcover_dataframe$fulldate[reference_number]))
    final_temp_residu = smoothed_temp_residuals - smoothed_temp_residuals[reference_number]
    
    corrected_temperature = landcover_dataframe$temperature + final_temp_residu
...
# Function that plots the evolution of temperature with and without temperature decline correction based on land cover applied
plot_temp_evolutions = function(temperature_dataframe){
  ...
  plot = ggplot(data = temperature_dataframe) + geom_line(aes(x = date, y = decline_corrected_no_smoothing, color = "Decline corrected no smoothing")) + geom_line(aes(x = date, y = inertia_corrected, color = "Inertia corrected (no decline)")) + geom_line(aes(x = date, y = decline_corrected, color = "Decline corrected"))
  plot2 = plot + scale_color_manual(values = c("Inertia corrected (no decline)" = "red","Decline corrected no smoothing" = "green", "Decline corrected" = "blue")) 
  plot3 = plot2 + labs(x = 'Time (in UTC)', y = 'Temperature (°C)') + theme(plot.title = element_text(hjust = 0.5), legend.position="top", legend.title=element_blank(), legend.text=element_text(size=17), axis.title=element_text(size=17), axis.text=element_text(size=17)) + guides(color=guide_legend(ncol=1))
  print(plot3)
  dev.off()
...
# The temperatures are corrected for temperature decline (based on the BBK land cover map of Flanders) and the route is plotted on the BBK land cover map of Flanders
